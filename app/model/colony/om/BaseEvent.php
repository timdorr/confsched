...
abstract class BaseEvent extends BaseObject  implements Persistent
{
	...
	public function setEnd($v)
	{
		...
		if ( $this->end !== null || $dt !== null ) {
			...
			$currNorm = ($this->end !== null && $tmpDt = new DateTime($this->end)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->end = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = EventPeer::END;
			}
		} // if either are not null
